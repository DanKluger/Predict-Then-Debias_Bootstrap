...
RandomSubsetThenLabelFixedSizesUniformSampling <- function(FormattedDatInp,mTarget,nLabelTarget,SampleWR=T){
    ...
    FormattedDatSubset$PiLabelUnsc <- rep(nLabelTarget/mTarget,mTarget)
...
source("BootstrapAndCalcEsts.glm.R")
...
colnames(PointEsts) <- c("Classical","Naive","PTD")
  ...
  for(i in 1:nSims){
    ...
    SimResultCurr <- PTDBootModularized(ProxyDat = CurrSamp$ProxyDat,GoodDat = CurrSamp$GoodDatWMissing ,BootstrapScheme=BootstrapSchemesInp,PiLabel = CurrSamp$PiLabelUnsc ,TuningScheme = tuningSchemesInp,RegType = regTypeInp,OutcomeVarName = outcomeVarName,alpha = alphaInp)
