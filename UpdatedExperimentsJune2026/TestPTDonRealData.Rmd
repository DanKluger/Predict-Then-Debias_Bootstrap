...
AlphaFoldSims <- runSimsRealData(FormattedDatInp = AlphaFoldFormatted,nSims = 2000,N_sampInp = 7500,nLabelTargInp = 1000,regTypeInp = "logistic",outcomeVarName = "IDR",BootstrapSchemesInp = c("FullBootPercentile","QuickConvolution","CLTBased"))
...
HousingPriceSims <- runSimsRealData(FormattedDatInp = HousingPriceFormatted,nSims = 2000,N_sampInp = 5000,nLabelTargInp = 500,regTypeInp = "linear",outcomeVarName = "HousingPrice",BootstrapSchemesInp = c("FullBootPercentile","QuickConvolution","CLTBased"))
...
HousingPriceSimsQuantileRegMedian <- runSimsRealData(FormattedDatInp = HousingPriceFormatted,nSims = 2000,N_sampInp = 5000,nLabelTargInp = 1000,regTypeInp = "Quantile Regression",outcomeVarName = "HousingPrice",BootstrapSchemesInp = c("QuickConvolution","FullBootPercentile"),tauForQuantileReg = 0.5)
...
TreecoverSims <- runSimsRealData(FormattedDatInp = TreecoverFormatted,nSims = 2000,N_sampInp = 5000,nLabelTargInp = 500,regTypeInp = "linear",outcomeVarName = "Treecover",BootstrapSchemesInp = c("FullBootPercentile","QuickConvolution","CLTBased"))
...
TreeCoverClusteredSimsAddressClustering <- runSimsRealData(FormattedDatInp = TreecoverFormatted,nSims = 2000,N_sampInp = 10000,nLabelTargInp = 1000,regTypeInp = "linear",outcomeVarName = "Treecover",clusteredLabelling = T,useClusterIDs = T,BootstrapSchemesInp = c("QuickConvolution","FullBootPercentile"))
...
TreecoverBinarizedSims <- runSimsRealData(FormattedDatInp = TreecoverBinarizedFormatted,nSims = 2000,N_sampInp = 8000,nLabelTargInp = 1000,regTypeInp = "logistic",outcomeVarName = "Forest",BootstrapSchemesInp = c("FullBootPercentile","QuickConvolution","CLTBased"))
...
Experiment #8 (Batch 1)

```{r}

load(paste0(getwd(),"/Datasets/CensusStratEducationFormatted.RData"))

CensusOrdinalRegSimsB1 <- runSimsRealData(FormattedDatInp = EducationDatFormatted,nSims = 1500,N_sampInp = 6000,nLabelTargInp = 1000,regTypeInp = "ordinal",outcomeVarName = "Education",tuningSchemesInp = c("Optimal","Diagonal","None"),BootstrapSchemesInp = c("FullBootPercentile","QuickConvolution"))

save(CensusOrdinalRegSimsB1,file = paste0(SimsResultPath,"CensusOrdinalRegSimsBatch1.RData"))
```

Batch 2 (the remaining 500 simulations were run on a computing cluster to speed up the process)

```{r}
#Loading both batches and merging them
SimsResultPath <- paste0(getwd(),"/SimulationResults/")
load(paste0(SimsResultPath,"CensusOrdinalRegSimsBatch1.RData"))
load(paste0(SimsResultPath,"CensusOrdinalRegSimsBatch2.RData"))


#Merging the batches
for(i in 1:length(CensusOrdinalRegSimsB1)){
  if(!identical(CensusOrdinalRegSimsB1[[i]],CensusOrdinalRegSimsB2[[i]])){print(names(CensusOrdinalRegSimsB1)[i])}
}

#total time should be summed and results list should be concatenated
totalTimeComb <- CensusOrdinalRegSimsB1[['totalTime']]+CensusOrdinalRegSimsB2[['totalTime']]
SimResultListComb <- c(CensusOrdinalRegSimsB1[['SimResultList']],CensusOrdinalRegSimsB2[['SimResultList']])


CensusOrdinalRegSims <- CensusOrdinalRegSimsB1
CensusOrdinalRegSims$totalTime <- totalTimeComb
CensusOrdinalRegSims$SimResultList <- SimResultListComb

save(CensusOrdinalRegSims,file = paste0(SimsResultPath,"CensusOrdinalRegSims.RData"))
```
