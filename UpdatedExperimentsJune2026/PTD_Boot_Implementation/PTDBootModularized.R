...
#  "ScalarTrace" Estimates optimal tuning matrix among scalar matrices times the identity (optimality is in terms of minimizing trace of asymptotic variance) 
...
PTDBootModularized <-function(ProxyDat,GoodDat,PiLabel=NULL,clusterID=NULL,OutcomeVarName="Y",RegType="linear",BootstrapScheme="QuickConvolution",nBootTune=0,nBootInference=2000,useInfBootForTuning=T,alpha=0.05, TuningScheme="Optimal",OmegaOutlierThresh=10^(10),tauQuantReg=NULL){
  ...
  N <- nrow(ProxyDat)
  p <- ncol(GoodDat)-1
  if(RegType %in% c("ordinal","Oridinal","Ordinal Regression","ordinal regression")){p <- p-1} #Don't consider intercept in ordinal regression
  ...
  if(!is.null(clusterID)){
    ...
    if(RegType %in% c("Quantile","Quantile Regression","Ordinal","Ordinal Regression")){
      print("Warning: Sandwich estimators for ordinal and quantile regression with clustered samples not available. Some of the standard errors could be wrong")
    }
  }
  
  if(RegType %in% c("Quantile","Quantile Regression","Ordinal","Ordinal Regression")){
    if((sum(BootstrapScheme %in% c("FullBootStudentized","CLTBased"))>0) ){
      print("Warning: Function doesn't support Studentized bootstrap, CLT based confidence intervals or CLT based tuning for quantile and ordinal regression.")
    }
  } 
  if(RegType %in% c("ordinal","ordinal regression","Ordinal","Ordinal Regression")){
    if(length(unique(PiLabel))>1){
    print("Warning: Current implementation of ordinal regression doesn't support weighted sample.")
    }
  }
...
#########    Running Bootstrap ######################################
  ...
  for(BootstrapSchemeCurr in BootstrapScheme){
    ...
    for(TuneSchemeCurr in TuningScheme){
      ...
      if(TuneSchemeCurr == "Diagonal"){
        DiagonalOmegaTuned <- diag(A12hat)/diag(A22hat+A33hat)
        DiagonalOmegaTuned[is.na(DiagonalOmegaTuned)] <- Inf 
        OmegaTuned <- diag(DiagonalOmegaTuned)
      }
      if(TuneSchemeCurr == "ScalarTrace"){
        lambdaTrace <- sum(diag(A12hat+t(A12hat)))/(2*sum(diag(A22hat+A33hat)))
        if(is.na(lambdaTrace)){lambdaTrace <- Inf}
        OmegaTuned <- lambdaTrace*diag(p+1)
      }
      if(TuneSchemeCurr == "Optimal"){
        if((sum(is.na(A12hat))>0) | (sum(is.na(A22hat))>0) ){ 
          OmegaTuned <- matrix(Inf,nrow = ncol(Boots$betaHatCalibBoot),ncol = ncol(Boots$gammaHatCalibBoot))
        }else if(rcond(A22hat+A33hat) > (.Machine$double.eps)){
      ...
      if(mean(abs(OmegaTuned))>OmegaOutlierThresh){
        print("Warning: Optimal omega estimate below likely too large. Setting to zero.")
        print(paste0("Original tuning scheme:",TuneSchemeCurr))
        if(TuneSchemeCurr!="CLTBasedTune"){print("Consider increasing nBootTune")}
      ...
      if(BootstrapSchemeCurr=="QuickConvolution"){
        ...
        if(sum(is.na(QuickPPEstsBoot))==0){
          CILBSUBS <- t(sapply(data.frame(QuickPPEstsBoot), FUN = function(x) quantile(x,probs = c(alpha/2,1-alpha/2))))
        } else{
          NABootdrawsCurr <- sum(!complete.cases(QuickPPEstsBoot))
          print(paste0("There were ",sum(!complete.cases(QuickPPEstsBoot))," bootstrap draws that had NAs. Setting CIs to NA."))
          CILBSUBS <- matrix(NA,ncol = 2,nrow = length(betaPPest))
        }
      ...
      if(BootstrapSchemeCurr=="FullBootPercentile"){
        ...
        if(sum(is.na(PPEstBoot))==0){
          ...
          print(paste0("There were ",NABootdrawsCurr," bootstrap draws that had NAs. Setting CIs to NA."))
          CILBSUBS <- matrix(NA,ncol = 2,nrow = length(betaPPest))
        }
        CICurrScheme <- cbind(matrix(betaPPest,ncol=1),CILBSUBS)
      }
      ...
      if(BootstrapSchemeCurr=="FullBasicBoot"){
        ...
        PivotsUnstdQuants <- t(sapply(data.frame(PivotsUnstd), FUN = function(x) quantile(x,probs = c(alpha/2,1-alpha/2)))) #If error here use conservative quantile intervals
