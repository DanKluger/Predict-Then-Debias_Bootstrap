...
EstBetaGammaCalib_andVCOV.glm <- function(GoodDatInp,ProxyDatInp,PiInp,RegType="linear",OutcomeVar="Y",Minp){
  ...
  D11hat <- t(matrix(rep(weightsX,p+1), ncol = (p+1),byrow = F)*X_augVal) %*% X_augVal/m
  D22hat <- t(matrix(rep(weightsXTilde,p+1), ncol = (p+1),byrow = F)*tildeX_augVal) %*% tildeX_augVal/m
  ...
  C11hat <- (t(WeightedScoreVecXmatVal) %*% WeightedScoreVecXmatVal)/m
  ...
  C22hat <- (t(WeightedScoreVecTildeXCalib) %*% WeightedScoreVecTildeXCalib)/m
  

  C12hat <- (t(WeightedScoreVecXmatVal) %*% WeightedScoreVecTildeXCalib)/m
