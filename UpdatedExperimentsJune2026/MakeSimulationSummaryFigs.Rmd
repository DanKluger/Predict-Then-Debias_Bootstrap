...
load(paste0(SimsPath,"CensusOrdinalRegSims.RData"))
...
ExperimentResultsAll[[8]] <- CensusOrdinalRegSims$SimResultList
...
CoefsNameMats[[8]]  <- cbind(c("Nativity","Age","Disability"),
                      c("Nativity","Age","Disability"))


rm(AlphaFoldSims,HousingPriceSims,HousingPriceSimsQuantileRegMedian,TreecoverSims,TreeCoverClusteredSimsAddressClustering,TreecoverBinarizedSims,SimResultsIncomeStratified,CensusOrdinalRegSims)
```
...
TrueBetaList[[8]] <- polr( Education~. ,data = EducationDatFormatted$GoodDat,Hess = T)$coefficients
TrueGammaList[[8]] <- polr( Education~. ,data = EducationDatFormatted$ProxyDat,Hess = T)$coefficients 

rm(AlphaFoldFormatted,HousingPriceFormatted,TreecoverFormatted,TreecoverBinarizedFormatted,IncomeDatFormatted,EducationDatFormatted)
```
...
Make Figure 2 (Half violin plots) NEED TO FORMAT TO ACCOUNT FOR NEW EXPERIMENT
```{r}
pltList <- list()
for(i in 1:8){
  if(i > 3){
...
titlenames <- c("(1) AlphaFold with Weighted Sampling","(2) Housing Price Linear Regression","(3) Housing Price Quantile Regression","(4) Tree Cover Linear Regression","(5) Tree Cover with Clustered Sampling","(6) Tree Cover Logistic Regression","(7) Census with Stratified Sampling","(8) Census Education Ordinal Regression")
...
pltExps4to7NoLegend <-  ggarrange(pltList[[4]],pltList[[5]],pltList[[6]],pltList[[7]],pltList[[8]],nrow = 5,labels = c(titlenamesTab[4],titlenamesTab[5],titlenamesTab[6],titlenamesTab[7],titlenamesTab[8]),font.label = list(size=8.5,face="plain"),vjust = 0.4,hjust=0)
pltExps4to8Legend   <-  ggarrange(pltExps4to7NoLegend,CombLegends,widths = c(3,1))

pltExps1to8 <- ggarrange(pltExps1to3NoLegend,pltExps4to8Legend,nrow = 2,heights = c(3.2,5))



  
ggsave(
   paste0(getwd(),"/Figures/Exps1to8HalfViolin.pdf"),
  plot = pltExps1to8,height = 8.5
)
```
