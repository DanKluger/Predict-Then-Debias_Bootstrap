...
FormatAndFitReg <- function(DatInp2,RegTypeInp2,WeightsInp=NULL,clusterIDInp2=NULL,OutcomeVarInp2="Y",CalcSandwhich=F,quantileTauInp=NULL){
  ...
  } else if(RegTypeInp2 %in% c("ordinal","Oridinal","Ordinal Regression","ordinal regression")){
    OrdinalRegFit <- polr(formula = RegFormulaLocal,data = DatInp2,Hess = T) #Using weights leads to issues with standard errors when running polr
    CoefsEst <- OrdinalRegFit$coefficients
    if(CalcSandwhich){VCOVSandwich <- sandwich(OrdinalRegFit)[names(CoefsEst),names(CoefsEst)]}
